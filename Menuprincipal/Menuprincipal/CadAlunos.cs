...
namespace Menuprincipal
{
    public partial class CadAlunos : Form
    {
         ...
         public CadAlunos(string cn)//cn= permissão de acesso ao form CadExterno
        {
            cnsql = cn;
            InitializeComponent();
        }
         ...
         {
             string sql = "insert into alunos (nome, email, rm) values (@nome, @email, @rm)";
             MySqlConnection conexao = new MySqlConnection(cnsql);//enviando caminho do banco
             MySqlCommand comando = new MySqlCommand(sql, conexao);//comparação do que foi digitado no form com a informação do banco

             comando.Parameters.Add("@nome", MySqlDbType.VarChar).Value = textBox1.Text;
             comando.Parameters.Add("@email", MySqlDbType.VarChar).Value = textBox2.Text;
             comando.Parameters.Add("@rm", MySqlDbType.VarChar).Value = textBox3.Text;
         ...
         private void editar()
         {
             ...
             string sql = "update alunos set nome=@nome, email=@email, rm=@rm where codigo='" + textBox4.Text + "'";
             MySqlConnection conexao = new MySqlConnection(cnsql);//enviando caminho do banco
             MySqlCommand comando = new MySqlCommand(sql, conexao);//comparação do que foi digitado no form com a informação do banco
         ...
         {
             string sql = "delete from alunos where codigo='" + textBox4.Text + "'";
             MySqlConnection conexao = new MySqlConnection(cnsql);//enviando caminho do banco
             MySqlCommand comando = new MySqlCommand(sql, conexao);//comparação do que foi digitado no form com a informação do banco
         ...
         private void consultarLIKE()
         {
             ...
             string sql = "select * from alunos where nome LIKE '%" + textBox5.Text + "%'";
             MySqlConnection conexao = new MySqlConnection(cnsql);//enviando caminho do banco
             MySqlCommand comando = new MySqlCommand(sql, conexao);//comparação do que foi digitado no form com a informação do banco
             ...
             if (leia.HasRows)//encontrou alguma coisa?
             {
                 ...
                 {
                     dataGridView1.Rows.Add(Convert.ToString(leia["codigo"]), Convert.ToString(leia["nome"]), Convert.ToString(leia["email"]), Convert.ToString(leia["rm"]));
                 }
             }
        ...
        private void CadAlunos_MouseMove(object sender, MouseEventArgs e)
        {
            if (textBox4.Text == "")
            {
                verificaIgual();
